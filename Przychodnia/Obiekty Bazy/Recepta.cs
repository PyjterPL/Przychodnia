...
namespace Przychodnia.Obiekty_Bazy
{
    class Recepta
    {
        ...
        public void DodajRecepte( int ID_lekarza, int ID_pacjenta, int ID_grafiku, string Tresc, DateTime Data_waznosci)
        {
            var zapytanie = string.Format("INSERT INTO recepty (Id_recepty,Id_lekarza,Id_pacjenta,Tresc,Data_waznosci,Id_grafiku) VALUES({0},{1},{2},{3},{4},{5}",
               null,ID_lekarza.ToString(),ID_pacjenta.ToString(),ID_grafiku.ToString(),Tresc.ToString(),Data_waznosci.ToString());
            var komenda = new MySqlCommand(zapytanie, DbHelper.Polaczenie);
